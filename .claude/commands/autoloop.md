...
The driver owns control flow, tests, and merge decisions. Claude is called only for implementation and the separate Ponytail review, using `claude -p --bare` to bypass local hooks/classifiers.
...
- Full `--run` starts with a Claude API smoke check before any branch or PR work.
...
`LOOP_REPO`, `LOOP_MODEL`, `LOOP_MAX_FIX`, `LOOP_MAX_REVIEW`, `LOOP_BARE`, `LOOP_CLAUDE_TIMEOUT_MS`, `LOOP_CLAUDE_SMOKE_TIMEOUT_MS`.
